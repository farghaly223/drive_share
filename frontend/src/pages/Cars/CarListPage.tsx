...
const CarListPage = () => {
  ...
  const fetchCars = async () => {
    try {
      ...
      // Fallback: if endpoint missing, show empty state
      setCars([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      <h2>My Cars</h2>
      <Link to="/cars/create" className="cta-button" style={{ display: 'inline-block', marginBottom: '1rem' }}>
        Add New Car
      </Link>
      {error && <ErrorAlert message={error} />}
      {cars.length === 0 && !error && (
        <p>You haven't listed any cars yet. Click "Add New Car" to get started.</p>
      )}
